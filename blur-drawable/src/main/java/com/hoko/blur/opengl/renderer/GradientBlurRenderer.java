...
import com.hoko.blur.opengl.shader.GradientShaderUtil;
 ...
 * 渐进模糊渲染器。
 *
 * 与 {@link ScreenBlurRenderer} 的差异：
 * <ul>
 *   <li>fragment shader 使用 {@link GradientShaderUtil#getGradientFragmentCode(int)}，
 *       内置"按屏幕 Y 动态推导 radius"的逻辑</li>
 *   <li>uniform {@code uRadius} 替换为 {@code uRadiusBase}（float），
 *       渲染时单次上传最大值即可</li>
 *   <li>其余两遍 pass 流程、缓存、纹理管理、矩阵推导完全沿用</li>
 * </ul>
 *
 * 调用方为 {@link com.hoko.blur.drawable.GradientBlurDrawable}，通过 {@link DrawFunctor}
 * 拿到的 {@link DrawFunctor.GLInfo} 触发 {@link #onDrawFrame(DrawFunctor.GLInfo, boolean)}。
 *
 * Created by hoko team.
 */
public class GradientBlurRenderer implements IRenderer<DrawFunctor.GLInfo> {

    private static final String TAG = GradientBlurRenderer.class.getSimpleName();
    ...
    private GradientBlurRenderer(Builder builder) {
        mMode = builder.mode;
    ...
    private boolean prepare() {
        ...
        if (mNeedRelink) {
            deletePrograms();
            mBlurProgram = ProgramFactory.create(
                    getVertexCode(),
                    GradientShaderUtil.getGradientFragmentCode(mMode));
        ...
        return checkGLError("Prepare to gradient blurring");
    }
    ...
    private boolean drawOneDimenBlur(float[] mvpMatrix, float[] texMatrix, boolean isHorizontal) {
        try {
            ...
            int radiusId = GLES20.glGetUniformLocation(mBlurProgram.id(), "uRadiusBase");
            ...
            GLES20.glUniform1f(radiusId, mRadius);
    ...
    public static class Builder {
        ...
        public Builder(GradientBlurRenderer screenRenderer) {
            Preconditions.checkNotNull(screenRenderer, "GradientBlurRenderer == null");
        ...
        public GradientBlurRenderer build() {
            return new GradientBlurRenderer(this);
        }
    }
