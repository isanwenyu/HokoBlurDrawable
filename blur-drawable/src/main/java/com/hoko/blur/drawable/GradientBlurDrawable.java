...
import com.hoko.blur.opengl.renderer.GradientBlurRenderer;
import com.hoko.blur.util.Preconditions;

/**
 * 渐进模糊 Drawable。
 *
 * 沿用 {@link BlurDrawable} 的 functor 钩子机制；与 BlurDrawable 的差异在于内部使用
 * {@link GradientBlurRenderer}：片段着色器中按屏幕 Y 比例动态推导模糊半径，
 * 实现"顶部 radius≈0、底部 radius≈{@code baseRadius}"的线性渐变效果。
 *
 * 用法：
 * <pre>{@code
 * GradientBlurDrawable drawable = new GradientBlurDrawable();
 * drawable.radius(20);
 * drawable.mode(GradientBlurDrawable.MODE_GAUSSIAN);
 * drawable.mixColor(0x80FFFFFF);
 ...
 */
public class GradientBlurDrawable extends Drawable {
    ...
    private GradientBlurRenderer mBlurRenderer;
    ...
    public GradientBlurDrawable() {
        mBlurRenderer = new GradientBlurRenderer.Builder().build();
